            ...
            foreach ($reader->getSheetIterator() as $sheet) {
                foreach ($sheet->getRowIterator() as $index => $row) {
                    ...
                    $rowData = $row->toArray();
                        ...
                        // Faculty, programme, Semester, New Subject, Course Code, Course Credit
                        $programme = $rowData[0] ?? null;
                        $semester = $rowData[1] ?? null;
                        $new_subject = $rowData[2] ?? null;
                        $course_code = $rowData[3] ?? null;
                        $course_credit = $rowData[4] ?? null;

                        // Basic validation
                        if (empty($programme) || empty($semester)
                            || empty($new_subject) || empty($course_code) || empty($course_credit)) {
                    ...
                    if ($pname !== false){
                        ...
                        $result = createSubject($new_subject, $pname, $semester, $course_code, $course_credit);;
;
                    if (is_bool($result)) {
                        ++$successCount;
                    } else {
