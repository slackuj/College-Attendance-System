...
// Get the modal
    var modal = document.getElementById("edit-programme");
...
pname.addEventListener('change',() => {
    btn2.disabled = pname.value === pnameOriginal;
});
...
// 1. Show loading state
    ...
    toastr.options.progressBar = true;
    toastr.options.timeOut = 2000;
    const message = `Updating Faculty`;
    toastr.info(message);
    ...
    try {
        ...
        const response = await fetch('/backend/api/edit_programme.php', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            if (data.success) {
                ...
                const toastData = {
                    type: 'success',
                    title: 'Successful!',
                    message: `Programme has been successfully updated.`
                };
        ...
        btn2.disabled = false;
        btn3.disabled = false;
        console.error("failed updating programme:", error);
        toastr.remove();
        toastr.options = {
            "positionClass": "toast-bottom-right",
            "closeButton": true,
