...
// Get the modal
    var modal = document.getElementById("reassign_class");
    var modal2 = document.getElementById("delete-class");
...
// Get the <span> element that closes the modal
   ...
   var btn2 = document.getElementById("assignBtn");
...
// When the user clicks anywhere outside of the modal, close it
    ...
    //  K E E P   A S S I G N   B U T T O N   D I S A B L E D
...
// 1. Show loading state
    ...
    toastr.options.positionClass = 'toast-bottom-right';
    toastr.options.progressBar = true;
    const message = `Reassigning Class`;
    toastr.info(message);
    ...
    try {
        ...
        const response = await fetch('/backend/api/reassign_class.php', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            if (data.success) {
                ...
                const toastData = {
                    ...
                    message: `Class has been reassigned successfully.`
                };
                ...
                btn2.disabled = false;
                btn3.disabled = false;
                console.error("failed reassigning class:", data.error);
                toastr.remove();
                toastr.options = {
                    "positionClass": "toast-bottom-right",
                    "closeButton": true,
                ...
                toastr.error(`${data.error}!`, 'Reassigning Class Failed');
            }
        } else {
...
// 1. Show loading state
        ...
        try {
            ...
            const response = await fetch('/backend/api/delete_class.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (response.ok) {
                ...
                if (data.success) {
                    ...
                    window.location.href = '/frontend/Admin/classes/classes.php';
                } else {
                    // Handle API-specific errors
                    btn5.classList.remove('loading');
                    btn5.disabled = false;
